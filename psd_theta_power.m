...
% 
% %% ===== Minimal: PSD theta power session table =====
% 
...
% % output table
% T_out = table( ...
%     strings(0,1), nan(0,1), nan(0,1), nan(0,1), strings(0,1), strings(0,1), ...
%     'VariableNames', {'rat','ss','theta_power_iHP','theta_power_mPFC','goal','stage'});
% 
...
%         if ~isfield(theta_TT, target_ss_id), continue; end
...
%             trial_time(i,1) = tick_timestamp(ue_Trialstart(i));
%             trial_time(i,2) = tick_timestamp(ue_Trialend(i));
%         end
% 
%         % iHP
%         iHPfile = [ROOT.Theta 'LE' char(rat) '\rat' char(rat) '-' ss '\AG' ...
%                    num2str(theta_info.bestTT_iHP) '_RateReduced_3-300filtered.ncs'];
%         if ~exist(iHPfile,'file'), continue; end
%         theta_iHP = local_sessionThetaPSD(iHPfile, trial_time, params.Fs, thetaBand, win, noverlap, nfft);
%         if isnan(theta_iHP), continue; end
% 
%         % mPFC
%         mPFCfile = [ROOT.Theta 'LE' char(rat) '\rat' char(rat) '-' ss '\AG' ...
%                     num2str(theta_info.bestTT_mPFC) '_RateReduced_3-300filtered.ncs'];
%         if ~exist(mPFCfile,'file'), continue; end
%         theta_mPFC = local_sessionThetaPSD(mPFCfile, trial_time, params.Fs, thetaBand, win, noverlap, nfft);
%         if isnan(theta_mPFC), continue; end
% 
%         % save
%         T_out = [T_out; {rat, ss_num, theta_iHP, theta_mPFC, string(SL.goal(k)), string(SL.stage(k))}]; 
% 
%     end
% end
% 
% save(fullfile(ROOT.Save,'theta_power_session_table_PSD.mat'), 'T_out');
